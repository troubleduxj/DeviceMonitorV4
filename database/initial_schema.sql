...
CREATE TABLE IF NOT EXISTS "dept_closure" (
    ...
    CONSTRAINT "uid_dept_closur_ancesto_523a57" UNIQUE ("ancestor_id", "descendant_id")
);
CREATE INDEX IF NOT EXISTS "idx_dept_closur_level_adb2a9" ON "dept_closure" ("level");
COMMENT ON COLUMN "dept_closure"."ancestor_id" IS '祖先ID';
COMMENT ON COLUMN "dept_closure"."descendant_id" IS '后代ID';
COMMENT ON COLUMN "dept_closure"."level" IS '深度';
