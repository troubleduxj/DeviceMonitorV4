...
const fetchTaskLogs = async () => {
  ...
  try {
    ...
    currentTaskLogs.value = '获取日志失败: ' + (error.message || '未知错误')
  } finally {
...
const fetchTasks = async () => {
  ...
  try {
    ...
    if (res && res.data && Array.isArray(res.data.items)) {
      ...
      // 增量更新逻辑
      if (tasks.value.length !== newTasks.length || !tasks.value.every((t, i) => t.id === newTasks[i].id)) {
        // 如果列表长度变化或ID顺序不一致，全量替换
        tasks.value = newTasks
      } else {
        // 否则只更新变化字段
        newTasks.forEach((newTask, index) => {
          const oldTask = tasks.value[index]
          if (oldTask.status !== newTask.status) oldTask.status = newTask.status
          if (oldTask.progress !== newTask.progress) oldTask.progress = newTask.progress
          if (oldTask.duration !== newTask.duration) oldTask.duration = newTask.duration
          // 可以根据需要更新其他字段
        })
      }
    }
